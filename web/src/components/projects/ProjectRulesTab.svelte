<script>
    ...
    export let project = new Collection();
    ...
    // all supported project rules in "project_rule_prop: label" format
    const ruleProps = {
        ...
        <div transition:slide|local={{ duration: 150 }}>
            <div class="alert alert-warning m-0">
                <div class="content">
                    ...
                    <p class="m-b-0">
                        You could also add constraints and query other projects using the <em
                            >@project</em
                        > filter:
                    </p>
                    <div class="inline-flex flex-gap-5">
                        <code>@project.ANY_COLLECTION_NAME.*</code>
                    </div>
...
{:else}
    {#each Object.entries(ruleProps) as [prop, label] (prop)}
        ...
        <div class="rule-block">
            ...
            <Field
                class="form-field rule-field m-0 {isAdminOnly(project[prop]) ? 'disabled' : ''}"
                name={prop}
                let:uniqueId
            >
                <label for={uniqueId} on:click={() => editorRefs[prop]?.focus()}>
                    {label} - {isAdminOnly(project[prop]) ? "Admins only" : "Custom rule"}
                </label>

                <svelte:component
                    this={ruleInputComponent}
                    bind:this={editorRefs[prop]}
                    bind:value={project[prop]}
                    baseProject={project}
                    disabled={isAdminOnly(project[prop])}
                />
