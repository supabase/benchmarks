<script>
    ...
    $: isProjectRenamed = project?.originalName != project?.name;

    $: renamedFields =
        project?.schema.filter(
            (field) => field.id && !field.toDelete && field.originalName != field.name
        ) || [];

    $: deletedFields = project?.schema.filter((field) => field.id && field.toDelete) || [];

    export async function show(projectToCheck) {
        project = projectToCheck;

        await tick();

        if (!isProjectRenamed && !renamedFields.length && !deletedFields.length) {
...
<OverlayPanel bind:this={panel} class="confirm-changes-panel" popup on:hide on:show>
    ...
    <div class="alert alert-warning">
        ...
        <div class="content txt-bold">
            <p>
                If any of the following changes is part of another project rule or filter, you'll have to
                update it manually!
            </p>
        ...
        {#if isProjectRenamed}
            <li>
                <div class="inline-flex">
                    Renamed project
                    <strong class="txt-strikethrough txt-hint">{project.originalName}</strong>
                    <i class="ri-arrow-right-line txt-sm" />
                    <strong class="txt"> {project.name}</strong>
                </div>
            </li>
        {/if}
