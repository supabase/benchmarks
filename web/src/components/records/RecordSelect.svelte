<script>
    ...
    async function loadSelected() {
        ...
        try {
            ...
            selected = await ApiClient.records.getFullList("benchmarks", 200, {
                sort: "-created",
                filter: (filters ? filters.join("||") + "&&" : "") + `benchmark_id="${project.id}"`,
    ...
    async function loadList(reset = false) {
        ...
        try {
            ...
            const result = await ApiClient.records.getList("runs", page, 200, {
                sort: "-created",
                $cancelKey: uniqueId + "loadList",
                filter: `benchmark_id="${projectId}"`,
            });
